...
;    ]
    if ticks = 0 [
;      file-open "hetero_various_ER_v1_initial.csv"
...
;        file-type ycor
;        file-type ","
;        file-print repressors
;      ]
;      file-close
      export-view "hetero_ellipse_v1_initial.png"
    ]

    if ticks = 3000 [
;      file-open "hetero_various_ER_v1_middle.csv"
;      ask repressors
;      [ file-type xcor
;        file-type ","
;        file-type ycor
...
;      ]
;      file-close
      export-view "hetero_ellipse_v1_middle.png"
    ]



    if ticks = 6000 [
;      file-open "hetero_various_ER_v1_final.csv"
;      ask repressors
;      [ file-type xcor
;        file-type ","
;        file-type ycor
...
;      ]
;      file-close
      export-view "hetero_ellipse_v1_final.png"
    ]
...
to re_active_transport
  ...
  let currposi (sqrt ((x1 + 5) * (x1 + 5) + y1 * y1) - cellsize_y / 6)
  let normalized  (currposi ^ 2) / ((currposi ^ 2) + (cellsize_y ^ 2 / 64)) + 0.1


  rt random-float 360

;  set temp_angle atan x1 y1
;  set temp_angle (90 - temp_angle) mod 360
;  if (0 <= temp_angle and temp_angle <= 60)[
;    set normalized 1]
;
  ;  if (210 <= temp_angle and temp_angle <= 270)[
  ;    set normalized 1]

  fd speedp * normalized


  set A (cellsize_x / 2 - size / 2)
  set B (cellsize_y / 2 - size / 2)


  let x2 xcor
  let y2 ycor

  if (sqrt ((x2 + 5)  * (x2 + 5)  + y2 * y2) - size / 2) < (cellsize_y / 6)
    [
      set radiii (cellsize_y / 6 + size / 2)
      let av ((x1 - x2) ^ 2 + (y1 - y2) ^ 2)
      let bv (-2 * ( (x2 - x1) * (x2 + 5)  + y2 ^ 2 - y1 * y2 ))
      let cv ((x2 + 5) ^ 2 + y2 ^ 2 - (cellsize_y / 6 + size / 2) ^ 2)
...
;
      let xt4 (xt + (yt ^ 2 - (xt + 5) ^ 2) / radiii ^ 2 * (x2 - xt) - 2 * (xt + 5) * yt / radiii ^ 2 * (y2 - yt))
      let yt4 (yt + ((xt + 5) ^ 2 - yt ^ 2) / radiii ^ 2 * (y2 - yt) - 2 * (xt + 5) * yt / radiii ^ 2 * (x2 - xt))
  ...
  if  (x2 * x2 / ((cellsize_x / 2 - size / 2) ^ 2) + y2 * y2 / ((cellsize_y / 2 - size / 2) ^ 2))  > 1
    [

      while [(x2 * x2 / ((cellsize_x / 2 - size / 2) ^ 2) + y2 * y2 / ((cellsize_y / 2 - size / 2) ^ 2))  > 1][
;      set radiii (cellsize / 2 - size / 2)
;      print (A)
      let av ((x1 - x2) ^ 2 / A ^ 2 + (y1 - y2) ^ 2 / B ^ 2)
      let bv (-2 * (x2 ^ 2 / A ^ 2 + y2 ^ 2 / B ^ 2 - x1 * x2 / A ^ 2 - y1 * y2 / B ^ 2))
      let cv (x2 ^ 2 / A ^ 2 + y2 ^ 2 / B ^ 2 - 1)
;      print (x1)
;      print (y1)
;      print (x2)
;      print (y2)
...
;      print (cv)
      ...
      let xt4 (xt + (yt ^ 2 / B ^ 2 - xt ^ 2 / A ^ 2) * (x2 - xt) - 2 * xt * yt / A / B * (y2 - yt))
      let yt4 (yt + (xt ^ 2 / A ^ 2 - yt ^ 2 / B ^ 2) * (y2 - yt) - 2 * xt * yt / A / B * (x2 - xt))
...
;  print (A)
;  if (sqrt (xcor ^ 2 / A ^ 2 + ycor ^ 2 / B ^ 2) > 1)[
;  print(x1)
;  print(y1)
;  print(x2)
;  print(y2)
...
to add2 [kind amount]
  create-turtles amount
    [ set breed kind
      setshape
      let radius1 random-float cellsize_x / 2
      let radius2 random-float cellsize_y / 2

      let thetha random-float 360
      setxy (radius1 * (cos thetha)) (radius2 * (sin thetha))


      while [sqrt ((xcor + 5) * (xcor + 5) + ycor * ycor) < (cellsize_y / 6 + size / 2 + 0.1) ][
        set radius1 random-float (cellsize_x / 2 - size / 2)
        set radius2 random-float (cellsize_y / 2 - size / 2)
...
to add3 [kind amount]
  create-turtles amount
    [ set breed kind
      setshape
      let radius random-float (cellsize_x / 2) - size / 2
      let thetha random-float 360
      ...
      while [dist + size / 2 > cellsize_x / 6 and dist - size / 2  < cellsize_x / 6]
      [
      set radius random-float (cellsize_x / 2) - size / 2
      set thetha random-float 360
