...
;      file-open "hetero_various_ER_v1_initial.csv"
      ...
      export-view "hetero_various_ER_v1_initial.png"
    ]

    if ticks = 3000 [

      export-view "hetero_various_ER_v1_middle.png"
    ]
    ...
    if ticks = 6000 [
      ...
      export-view "hetero_various_ER_v1_final.png"
    ]
...
to re_active_transport
  ...
  let currposi (sqrt (x1 * x1 + y1 * y1) - cellsize / 6)
  let normalized  (currposi ^ 2) / ((currposi ^ 2) + (cellsize ^ 2 / 36)) + 0.1
...
;      print (cv)
  ...
  if sqrt (xcor ^ 2 + ycor ^ 2) > (cellsize / 2 + size / 2)[
  print(x1)
  print(y1)
  print(x2)
  print(y2)
...
to add2 [kind amount]
  create-turtles amount
    [ set breed kind
      setshape
let radius sqrt( (cellsize / 6 + size / 2) ^ 2 +  (random-float 1 ) *  ((cellsize / 2 - size / 2) ^ 2 - (cellsize / 6 + size / 2) ^ 2))
